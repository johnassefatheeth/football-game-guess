...
if option == "Specific Game":
    ...
    if st.button("Predict"):
        if home_team not in df['HomeTeam'].values or away_team not in df['AwayTeam'].values:
            st.write("Invalid team names. Please try again.")
        else:
            ...
            home_goals_pred = model_home_goals.predict(X)
            away_goals_pred = model_away_goals.predict(X)
            mse_home = mean_squared_error(y_home_goals, home_goals_pred)
            mse_away = mean_squared_error(y_away_goals, away_goals_pred)
            r2_home = r2_score(y_home_goals, home_goals_pred)
            r2_away = r2_score(y_away_goals, away_goals_pred)

            # Step 5: Display Prediction Result and Evaluation Metrics
            st.write(f"Prediction: {home_team} {home_goals} : {away_goals} {away_team}")
            st.write(f"MSE for home goals: {mse_home:.2f}")
            st.write(f"MSE for away goals: {mse_away:.2f}")
            st.write(f"R-squared score for home goals: {r2_home:.2f}")
            st.write(f"R-squared score for away goals: {r2_away:.2f}")
